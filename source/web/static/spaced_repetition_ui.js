export function createSpacedRepetitionUi({ send }) {
  ...
  function renderMoveList(tree) {
    ...
    root.addEventListener("click", (event) => {
        console.log(`[DEBUG] Root click listener fired`);
        event.stopPropagation();
        console.log(`[DEBUG] stopPropagation() called in root`);
    ...
    for (const { node, branchDepth } of items) {
      ...
      el.addEventListener("click", (event) => {
        console.log(`[DEBUG] Click listener fired for path: ${JSON.stringify(path)}`);
        console.log(`[DEBUG] Event target tagName:`, event.target.tagName);
        console.log(`[DEBUG] Event currentTarget tagName:`, event.currentTarget.tagName);
        console.log(`[DEBUG] Element class:`, event.currentTarget.className);
        event.stopPropagation();
        console.log(`[DEBUG] stopPropagation() called for path: ${JSON.stringify(path)}`);
        send({ type: "sr_goto", path });
      });
